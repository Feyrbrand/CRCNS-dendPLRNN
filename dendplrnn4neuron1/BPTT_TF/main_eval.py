...
class EvaluateKLx(Evaluator):
    def __init__(self, init_data, inp_size):
    ...
    def metric(self, model):
        ...
        inp_rs = data_reshaped[:,:,-self.inp_size:]
        dat_rs = data_reshaped[:,:,:-self.inp_size]

        if len(inp_rs.shape)<3:
            inp_rs = inp_rs.unsqueeze(2)
        if len(dat_rs.shape)<3:
            dat_rs = dat_rs.unsqueeze(2)

        for (inp,traj) in zip(inp_rs,dat_rs):
            X, Z = model.generate_free_trajectory(inp, traj, T_sub)
            trajs.append(X)
...
class EvaluateMSE(Evaluator):
    ...
    def metric(self, model):
        mse_results = mse.n_steps_ahead_pred_mse(model, self.data, self.inp_size, n_steps=self.n_steps)
...
class EvaluatePSE(Evaluator):
    ...
    def metric(self, model):
        ...
        data = self.data[:, :-self.inp_size]
        inp_data = self.data[:, -self.inp_size:]
        ...
        self.data = data.to(model.device)
        data_gen, _ = model.generate_free_trajectory(inp_data, self.data, len(self.data))
...
def choose_evaluator_from_metric(metric_name, init_data, inp_size):
    if metric_name == 'mse':
        EvaluateMetric = EvaluateMSE(init_data, inp_size)
    elif metric_name == 'klx':
        EvaluateMetric = EvaluateKLx(init_data, inp_size)
    elif metric_name == 'pse':
        EvaluateMetric = EvaluatePSE(init_data, inp_size)
    else:
        raise NotImplementedError
    return EvaluateMetric


def eval_model_on_data_with_metric(model, data, inp_size, metric):
    init_data = (None, data, None)
    #print(init_data.shape)
    EvaluateMetric = choose_evaluator_from_metric(metric, init_data, inp_size)
