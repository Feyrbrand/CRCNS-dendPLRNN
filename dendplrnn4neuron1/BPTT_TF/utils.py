...
def load_dataset(args):
    # TODO - replace hardcoded 100 by some other value..
    data = read_data(args.data_path)
    data_set = dataset.GeneralDataset(data, args.seq_len, args.batch_size, args.batches_per_epoch)

    # if there is input in the dataset, this is saved in the last n dimensions
    args.dim_x = data.shape[1] - args.input_size

    # make sure the dataset is divisible by 100 (?)
    data_set = data_set
