...
for i in range(13):
    ...
    file_train = h5py.File("data/challengeAfull_val_39.hdf5")
    ...
    for epoch in np.arange(start, stop, step):  # Starts at 25, ends at 5000, steps of 25
        print(f"epoch: {epoch}")
...
filename_data = f"{directory}/all_data_spikes_val_{start}_{stop}.npy"
np.save(filename_data, all_data_spikes)
filename_gammas = f"{directory}/all_model_gammas_val_{start}_{stop}.npy"
np.save(filename_gammas, all_model_gammas)
filename_best = f"{directory}/best_model_performance_val_{start}_{stop}.npy"
