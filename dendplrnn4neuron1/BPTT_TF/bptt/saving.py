...
class Saver:
    def __init__(self, writer, save_path, args, data_set, inp_size, regularizer):
        self.writer = writer
    ...
    def epoch_save(self, model, epoch):
        ...
        if self.args.use_tb:
            with tc.no_grad():
                ...
                path = os.path.join(self.save_path, f'model_{epoch}.pt')
    ...
    def save_loss_terms(self):
        ...
        with tc.no_grad():
            input_.unsqueeze_(0)
            pred = self.current_model(input_[:,:,:-1], input_[:,:,-1:],self.args.teacher_forcing_interval)
            loss = nn.functional.mse_loss(pred, target[:,:-1].unsqueeze_(0))
        ...
        # A norm
        A, W, h, C = latent_model_parameters
    ...
    def save_metrics(self):
        ...
        for metric in self.args.metrics:
            ...
            metric_value = main_eval.eval_model_on_data_with_metric(model=self.current_model, data=data_subset, inp_size=self.inp_size, metric=metric)
    ...
    def save_prediction(self):
        ...
        self.current_model.plot_prediction(data,self.inp_size)
