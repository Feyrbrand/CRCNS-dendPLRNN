...
class BPTT:
    def __init__(self, args: Namespace, data_set: GeneralDataset,
        ...
        self.saver = saving.Saver(writer, save_path, args, self.data_set, self.inp_size, self.regularizer)
    ...
    def train(self):
        ...
        for epoch in range(1, self.n_epochs + 1):
            ...
            # the inp should include stim, but not the target..
            for idx, (inp, target) in enumerate(dataloader):
                self.optimizer.zero_grad(set_to_none=True)
                inp += tc.randn_like(inp) * self.noise_level

                x_in = inp[:,:,:-self.inp_size]
                x_out = target[:,:,:-self.inp_size]

                if len(x_in.shape) < 3:
                    x_in = x_in[:,:,np.newaxis]
                    x_out = x_out[:,:,np.newaxis]

                s_in = inp[:,:,-self.inp_size:]
                if len(s_in.shape) < 3:
                    s_in = s_in[:,:,np.newaxis]

                pred = self.model(x_in, s_in, self.tau)

                loss = self.compute_loss(pred, x_out)
