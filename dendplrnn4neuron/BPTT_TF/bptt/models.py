...
class Model(nn.Module):
    def __init__(self, args=None, data_set=None):
        ...
        if args is not None:
            ...
            self.args = vars(args)
            self.init_from_args()
            self.inp_size = args.input_size

    def forward(self, x, s, tau=None, z0=None):
        B = None
        ...
        hidden_out = self.latent_model(x, s, tau, z0, B)
    ...
    @tc.no_grad()
    def generate_free_trajectory(self, inp: tc.Tensor, data: tc.Tensor, T: int,
                                 z0: tc.Tensor = None):
        B = None
        # teacher forcing with obs. model inversion
        if self.args['use_inv_tf']:
        ...
        #print(self.z0_model.dtype)
        if self.z0_model and z0 is None:
            z0 = self.z0_model(data[0, :].unsqueeze_(0))
        # latent traj is T x dz
        latent_traj = self.latent_model.generate(inp, T, data, z0, B)
    ...
    def plot_simulated(self, data: tc.Tensor, T: int):
        ...
        x_data = data[:, :-self.inp_size]
        inp_data = data[:, -self.inp_size:]
        ...
        X, Z = self.generate_free_trajectory(inp_data, x_data, T)
        fig = plt.figure()
        plt.title('simulated')
        plt.axis('off')
        plot_list = [X, Z]
    ...
    def plot_obs_simulated(self, data: tc.Tensor):
        ...
        plt.axis('off')
        n_units = data.shape[1] - self.inp_size
    ...
    @tc.no_grad()
    def plot_prediction(self, data: tc.Tensor, inp_size: int,
        ...
        if rand_seq:
            ...
            t = 0
        input_ = data[t : t + T].unsqueeze(0)

        x_data = input_[:, :, :-inp_size]
        inp_data = input_[:, :, -inp_size:]

        if len(x_data.shape)<3:
            x_data = x_data.unsqueeze(2)
        if len(inp_data.shape)<3:
            inp_data = inp_data.unsqueeze(2)

        pred = self(x_data, inp_data, N).squeeze(0)
        ...
        for i in range(max_units):
            fig.add_subplot(max_units, 1, i + 1)
            plt.plot(x, x_data[0, 1:, i].cpu(), label='GT')
            ax = plt.gca()
            lim = ax.get_ylim()
            plt.plot(x, pred[:-1, i].cpu(), label='Pred')
            ax.set_ylim(lim)
            plt.scatter(x[::N], x_data[0,1::N, i].cpu(), marker='2',
...
class Z0Model(nn.Module):
    ...
    def __init__(self, dx: int, dz: int):
        ...
        self.MLP = nn.Linear(dx, dz-dx, bias=False).float()
